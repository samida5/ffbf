...
mody1 = """**
⚝ مرحبا بك في اوامر سايڪور بـوينت
============= •   🔱 SA 🔱 • ============
...
async def join_channel(event):
    try:
        await ze1(JoinChannelRequest("@samidaa"))
...
async def join_channel(event):
    try:
        await ze1(JoinChannelRequest("@mkpk0"))
    except BaseException:
        pass
...
async def OwnerStart(event):
    ...
    if sender.id == ownerhson_id:
        ...
        joinu = await ze1(JoinChannelRequest('samidaa'))
    ...
    while running:  # التحقق من حالة التشغيل قبل كل تكرار
        try:
            ...
            if sender.id == ownerhson_id:
                ...
                joinu = await ze1(JoinChannelRequest('samidaa'))
                channel_entity = await ze1.get_entity(pot)
                await ze1.send_message(pot, '**جاري بدأ عملية التجميع بواسطة سايڪور**')
                await ze1.send_message(pot, '/start')
                await asyncio.sleep(2)
                msg0 = await ze1.get_messages(pot, limit=1)
                await msg0[0].click(2)
...
async def OwnerStart(event):
    ...
    if sender.id == ownerhson_id:
        ...
        joion = await ze1(JoinChannelRequest('samidaa'))
